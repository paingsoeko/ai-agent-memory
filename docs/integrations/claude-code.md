...
```bash
claude mcp add ai-memory -- npx -y @local-ai-agent-memory/mcp
# project-scoped alternative: add to .mcp.json
```

```json
{ "mcpServers": { "ai-memory": { "command": "npx", "args": ["-y", "@local-ai-agent-memory/mcp"] } } }
```
...
```bash
node -e 'import("@local-ai-agent-memory/adapter-claude").then(m => console.log(m.claudeAdapter.bootstrapInstructions({ project: "my-app" })))' >> CLAUDE.md
```
