...
```json
{
  ...
  "mcp": {
    "ai-memory": { "type": "local", "command": ["npx", "-y", "@ai-memory/mcp"], "enabled": true }
  }
}
```
...
Append `opencodeAdapter.bootstrapInstructions()` (from `@ai-memory/adapter-opencode`) to `AGENTS.md`.
