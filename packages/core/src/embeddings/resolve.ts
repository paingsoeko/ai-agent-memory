 ...
 * `@ai-memory/embeddings` (which registers "local", "ollama", "openai", ...).
 */
export async function resolveEmbeddingProvider(
  ...
  if (!factory) {
    try {
      ...
      await import("@ai-memory/embeddings" as string);
    } catch {
      /* not installed */
    }
  ...
  if (!factory) {
    throw new MemoryError(
      ...
      'Install @ai-memory/embeddings for "local", "ollama" and "openai", or set embeddings.provider to "hash" / "none".',
    );
  }
